    ...
    public partial class Form1 : Form
    {
        ...
        public async Task CheckForUpdates()
        {
            try
            {
                ...
                //If there are mods that need updates, request an update
        ...
        private async Task UpdateMod(string modName)
        {
            ...
            {
                if (!f.Contains('.')) // Is a sub folder
                {
                    List<string> l = await Task.Run(() => DirFiles(serverModPath + f));
                    sFiles.Add(modName + "\\" + l[0].Replace('/', '\\'));
                }
                else //Is a primary file
                    sFiles.Add(f);
            }
            ...
            foreach (string fi in clientFiles)
            {
                if (fi.Contains('.'))
                {
                    if (!sFiles.Contains(fi.Substring(fi.LastIndexOf('@')))) // Primary File
                    {
                        if (File.Exists(fi))
                            File.Delete(fi);
                        else
                            MessageBox.Show("Error finding/deleting unnecessary file");
                    }
                }
            }
            ...
            Parallel.ForEach(files, file =>
           {
               ...
               {
                   DirectoryInfo di = Directory.CreateDirectory(installDir + "\\" + modName);
               }

               if (file.Contains("."))
               {
                   string clientPath = installDir + "\\" + modName + "\\" + file.Substring(file.LastIndexOf("/") + 1);
        ...
        private void CreateUserData(bool createFolder)
        {
            ...
            string[] lines =
            {
            "null", "null", "false", "false", "false", "Default"
            };
        ...
        public void LaunchArma()
        {
            try
            {
                ...
                foreach (string mod in mods)
                {
                    if (modDirTextBox.Text != armaDir && mod != "")
                        if (i >= 1)
                ...
                if (profileBox.Text != "Default")
                    launchPars += $"\n-name={profileBox.Text}";

                string modArgs = $@"{string.Join("", modSet)}";

                MessageBox.Show($"{armaDir}\\arma3battleye.exe " + $@"-mod=""{modArgs};""");

                System.Diagnostics.Process.Start($"{armaDir}\\arma3battleye.exe", $@"-mod=""{modArgs};""");
                this.Close();
            }
