...
namespace StowMarines_Interface
{
    public partial class Form1 : Form
    {
        ...
        private async void Form1_Load(object sender, EventArgs e)
        {
            ...
            cpuCountBox.Items.Add(" ");
            cpuCountBox.Items.Add("3");
            cpuCountBox.Items.Add("5");
            cpuCountBox.Items.Add("7");

            exThreadsBox.Items.Add(" ");
            exThreadsBox.Items.Add("0");
            exThreadsBox.Items.Add("1");
            exThreadsBox.Items.Add("3");
            exThreadsBox.Items.Add("5");
            exThreadsBox.Items.Add("7");
        ...
        {
            if (mainMenuBox.Visible == true)
                OpenMenu("close");
            else
                OpenMenu("launch");
        }

        private void settingsCog_Click(object sender, EventArgs e)
        {
            if (DelModsBox.Visible == true)
                OpenMenu("close");
            else
                OpenMenu("settings");
        }
        ...
        private void cpuCountBox_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            string[] arrLine = File.ReadAllLines(dataFolder + "//userData.txt");
            arrLine[8] = cpuCountBox.Text;
            File.WriteAllLines(dataFolder + "//userData.txt", arrLine);
        }

        private void exThreadsBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            string[] arrLine = File.ReadAllLines(dataFolder + "//userData.txt");
            arrLine[9] = exThreadsBox.Text;
            File.WriteAllLines(dataFolder + "//userData.txt", arrLine);
        }

        private void maxMemUpDown_ValueChanged(object sender, EventArgs e)
        {
            string[] arrLine = File.ReadAllLines(dataFolder + "//userData.txt");
            arrLine[10] = Math.Round(maxMemUpDown.Value).ToString();
            File.WriteAllLines(dataFolder + "//userData.txt", arrLine);
        }

        private void DelModsBox_CheckedChanged(object sender, EventArgs e)
        {
            string[] arrLine = File.ReadAllLines(dataFolder + "//userData.txt");
            if (DelModsBox.Checked)
            {
                arrLine[11] = "true";
                delMods = true;
            }
        ...
        public async Task CheckForUpdates()
        {
            try
            {
                ...
                string modString = Encoding.UTF8.GetString(modsRequest.DownloadData(url1));
                string[] mods = modString.Split('\n');

                WebClient sizeRequest = new WebClient();
                string url2 = "ftp://188.165.239.165/" + "modSizes.txt";
                sizeRequest.Credentials = credentials;
                string sizeString = Encoding.UTF8.GetString(modsRequest.DownloadData(url2));
                ...
                else if (timesRan > 1)
                {
                    ...
                    _ = Invoke(new Action(() =>
                    {
                        ...
                        armaDirButton.Enabled = true;
                        modDirButton.Enabled = true;
        ...
        private async Task UpdateMod(string modName)
        {
            ...
            Parallel.ForEach(files, file =>
           {
               ...
               if (!Directory.Exists(path))
               {
                   DirectoryInfo di = Directory.CreateDirectory(path); //Create mod folder if it doesn't exist
               }
               if (file.Contains(".")) //Primary file
               {
        ...
        private async void CheckDirs()
        {
            ...
            {
                MessageBox.Show("Please set your Arma 3 Directory", "Arma Directory is not set!", 
                    MessageBoxButtons.OK, MessageBoxIcon.None, MessageBoxDefaultButton.Button1, (MessageBoxOptions)0x40000);
                _ = Invoke(new Action(() =>
                {
                    SetArmaDir();
                }));
        ...
        private void CreateUserData(bool createFolder)
        {
            ...
            string[] lines =
            {
            "null", "null", "false", "false", "false", "Default", "true", "false", "null", "null", "0", "false"
            };
            ...
            _ = Invoke(new Action(() =>
            {
                customModDirLabel.Text = "Same as above";
            }));
        ...
        private void DelMods(string[] sMods)
        {
            /*List<string> cMods = new List<string>(Directory.GetDirectories(installDir, "@*", SearchOption.TopDirectoryOnly));

            foreach (string m in cMods)
            {
                int pos = Array.IndexOf(sMods, m.Substring(m.LastIndexOf('@')));
                MessageBox.Show(m + pos);
                if (pos == -1 && Directory.Exists(m))
                {
                    MessageBox.Show(m);
                    //Directory.Delete(m);
                }
        ...
        private async void GetUserData()
        {
            ...
            if (lines.Length != 12 || lines.Contains(null))
            {
                await Task.Run(() => CreateUserData(false));
            }  
            else
            {
                ...
                if (lines[7] == "true")
                    mainMenuBox.Checked = true;

                if (lines[8] == "null")
                    cpuCountBox.SelectedIndex = 0;    
                
                if (lines[8] == "3")
                    cpuCountBox.SelectedIndex = 1;

                if (lines[8] == "5")
                    cpuCountBox.SelectedIndex = 2;

                if (lines[8] == "7")
                    cpuCountBox.SelectedIndex = 3;

                if (lines[9] == "null")
                    exThreadsBox.SelectedIndex = 0;

                if (lines[9] == "0")
                    exThreadsBox.SelectedIndex = 1;

                if (lines[9] == "1")
                    exThreadsBox.SelectedIndex = 2;

                if (lines[9] == "3")
                    exThreadsBox.SelectedIndex = 3;

                if (lines[9] == "5")
                    exThreadsBox.SelectedIndex = 4;

                if (lines[9] == "7")
                    exThreadsBox.SelectedIndex = 5;

                if (lines[10] != "0")
                    maxMemUpDown.Value = Convert.ToDecimal(lines[10]);

                if (lines[11] == "true")
                {
                    DelModsBox.Checked = true;
                    delMods = true;
                } 
        ...
        private async void SetArmaDir()
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                string[] arrLine = File.ReadAllLines(dataFolder + "//userData.txt");
                arrLine[0] = armaDir;
                await Task.Run(() => File.WriteAllLines(dataFolder + "//userData.txt", arrLine));
                restart();
            }            
        }

        private async void SetModDir(bool state)  
        {
            if (state)
            {
                FolderBrowserDialog folderDlg = new FolderBrowserDialog();
        ...
        {
            LaunchTitle.Visible = false;
            OptionsTitle.Visible = false;
            optionsPanel1.Visible = false;
            optionalModsBox.Visible = false;
            profileBox.Visible = false;
            mainMenuBox.Visible = false;
            noPauseBox.Visible = false;
            noSplashBox.Visible = false;
            hugePagesBox.Visible = false;
            cpuCountLabel.Visible = false;
            exThreadsLabel.Visible = false;
            maxMemLabel.Visible = false;
            cpuCountBox.Visible = false;
            exThreadsBox.Visible = false;
            maxMemUpDown.Visible = false;
            mbLabel.Visible = false;
            DelModsBox.Visible = false;

            if (type == "launch")
            {
                LaunchTitle.Visible = true;
                optionsPanel1.Visible = true;
                LaunchTitle.Text = "Launch Parameters";
                optionalModsBox.Visible = true;
                profileBox.Visible = true;
                mainMenuBox.Visible = true;
                noPauseBox.Visible = true;
                noSplashBox.Visible = true;
                hugePagesBox.Visible = true;
                cpuCountLabel.Visible = true;
                exThreadsLabel.Visible = true;
                maxMemLabel.Visible = true;
                cpuCountBox.Visible = true;
                exThreadsBox.Visible = true;
                maxMemUpDown.Visible = true;
            ...
            else if (type == "settings")
            {
                ...
                optionsPanel1.Visible = true;
                LaunchTitle.Text = "App Settings";
                DelModsBox.Visible = true;
            }
        }
        ...
        public void LaunchArma()
        {
            try
            {
                ...
                string[] mods = Encoding.UTF8.GetString(launchRequest.DownloadData(url1)).Split(';');
                ...
                if (cpuCountBox.Text == "3" || cpuCountBox.Text == "5" || cpuCountBox.Text == "7")
                    launchPars += $" -cpuCount={cpuCountBox.Text}";

                if (exThreadsBox.Text == "0" || exThreadsBox.Text == "1" || exThreadsBox.Text == "3" || exThreadsBox.Text == "5" || exThreadsBox.Text == "7")
                    launchPars += $" -exThreads={cpuCountBox.Text}";

                if (profileBox.Text != "Default")
                    launchPars += $@""" -name={profileBox.Text}""";

                if (maxMemUpDown.Value >= 1024)
                    launchPars += $" -maxMem={Math.Round(maxMemUpDown.Value)}";
                ...
                if (optionalMods)
                {
                    ...
                    launchRequest.Credentials = credentials;
                    string[] opMods = Encoding.UTF8.GetString(launchRequest.DownloadData(url2)).Split(';');
